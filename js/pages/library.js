import { state, addFile, updateFile, persist } from '../store.js'
import { uploadFile } from '../drive-store.js'
...
function renderUploadModal(open) {
  ...
  document.getElementById('upload-form').addEventListener('submit', async (e) => {
    ...
    try {
      const driveFileId = await uploadFile(selected, selected.name, selected.type)
      updateFile(meta.id, { driveFileId, driveStatus: 'synced' })
      showToast('File uploaded — synced to your Drive', 'success')
    } catch (err) {
      updateFile(meta.id, { driveStatus: 'failed' })
      showToast('Upload to Drive failed. You can retry from the library.', 'error')
    }
