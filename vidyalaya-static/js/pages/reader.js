import { state, markFileStatus, getFileBlob, addNote, getNotesForFile, deleteNote, addBookmark, getBookmarksForFile, deleteBookmark, persist } from '../store.js'
...
async function loadViewer(file) {
  ...
  if (!blob) {
    viewport.innerHTML = `<p class="text-sm text-ink/40 dark:text-paper/40">No preview available.</p>`
    return
  }
